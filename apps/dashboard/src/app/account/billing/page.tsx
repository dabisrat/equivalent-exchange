...
import { getStripe, getOrCreateStripeCustomerForUser } from '@eq-ex/shared/stripe'
...
export default async function BillingPage() {
  ...
  async function createCheckout() {
    ...
    const customerId = await getOrCreateStripeCustomerForUser({ userId: user.id, email: user.email ?? null })
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="space-x-2">
        {!isPro ? (
          ...
          >
            <button type="submit" className="px-4 py-2 rounded bg-black text-white">Upgrade to Pro</button>
          </form>
        ) : (
          <form
            action={openPortal}
          >
            <button type="submit" className="px-4 py-2 rounded bg-black text-white">Manage Billing</button>
          </form>
        )}
      </div>
    </div>
