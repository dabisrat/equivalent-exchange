...
export async function getOrganizationBySubdomain(
  ...
  if (!isValidSubdomainFormat(subdomain)) {
    return {
      ...
      message:
        "Invalid subdomain format. Subdomains must be 3-50 characters, contain only letters, numbers, and hyphens, and cannot start or end with a hyphen.",
      subdomain,
    };
  }
  ...
  try {
    ...
    if (!data.is_active) {
      return {
        ...
        error: "INACTIVE",
        message:
          "This organization is temporarily unavailable. Please contact the organization administrator or try again later.",
        subdomain,
      };
    }
    ...
    return {
      ...
      message:
        "Unable to load organization data. Please try again in a few moments.",
      subdomain,
    };
  }
}
