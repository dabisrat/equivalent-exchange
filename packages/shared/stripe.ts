...
export async function mapCustomerToUser(customerId: string): Promise<string | null> {
  const supabase = await createClient(true)
...
export async function syncStripeDataForCustomer(customerId: string): Promise<SyncResult> {
  const stripe = getStripe()
  const supabase = await createClient(true)
  ...
  if (subs.data.length === 0) {
    ...
    if (userId) {
      await supabase
        .from('user_subscriptions')
        .upsert({ user_id: userId, stripe_customer_id: customerId, status: 'none' }, { onConflict: 'user_id' })
    }
    return { status: 'none' }
  }
    ...
    await supabase.from('user_subscriptions').upsert(
      {
        ...
        status: sub.status,
        price_id: priceId,
        current_period_start: new Date((((sub as any).current_period_start ?? 0) as number) * 1000).toISOString(),
        current_period_end: new Date((((sub as any).current_period_end ?? 0) as number) * 1000).toISOString(),
